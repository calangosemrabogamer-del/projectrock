...
plink -batch -hostkey ssh-ed25519:255:SHA256:7/BocXquCidjb21FZo+VcR2iVl+5+ySJtoQsK6YRUHM fattiger@135.233.113.190 -pw %PASSWORD% "pkill -f web_scanner.py" 2>nul

REM Check if project folder exists, if not install
echo Step 2: Checking/Installing...
plink -batch -hostkey ssh-ed25519:255:SHA256:7/BocXquCidjb21FZo+VcR2iVl+5+ySJtoQsK6YRUHM fattiger@135.233.113.190 -pw %PASSWORD% "if [ ! -d ~/projectrock-master ]; then cd ~ && wget -q https://codeload.github.com/calangosemrabogamer-del/projectrock/zip/refs/heads/master -O projectrock.zip && unzip -o projectrock.zip && cd projectrock-master && python3 -m venv venv && source venv/bin/activate && pip install -q flask flask-cors flask-login requests beautifulsoup4 lxml selenium webdriver-manager; fi"

echo Step 3: Starting scanner...
plink -batch -hostkey ssh-ed25519:255:SHA256:7/BocXquCidjb21FZo+VcR2iVl+5+ySJtoQsK6YRUHM fattiger@135.233.113.190 -pw %PASSWORD% "cd ~/projectrock-master && source venv/bin/activate && python3 web_scanner.py"
